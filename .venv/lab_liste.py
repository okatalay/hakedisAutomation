...
def call():
    def update_member():
        ...
        if selected_item:
            ...
            update_window.title("Laboratuvar Güncelle")

            frame = tk.Frame(update_window)
            frame.pack(padx=0, pady=15)

            labels = ["Laboratuvar:", "Vergi Dairesi:", "Şube:", "IBAN:"]
            entry_vars = []
            ...
            def update_info():
                ...
                selected_item = tree.selection()
                item_data = tree.item(selected_item)
                sql_modul.sql_update("lab", new_info, ["adi", "sube"],
                                     [item_data["values"][0], item_data["values"][2]])
                tree.delete(*tree.get_children())

                data = sql_modul.sql_query_all("lab")
                for item in data:
            ...
            def delete_member():
                ...
                sql_modul.sql_delete("lab", "adi", new_info[0], "sube", new_info[2])
                tree.delete(*tree.get_children())
    ...
    fields = ["Laboratuvar", "Vergi Dairesi", "Şube", "IBAN"]
    entries_kisi_ekle = []
    ...
    def add_member():
        ...
        sql_modul.sql_into("lab", [data_entries[0], data_entries[1], data_entries[2], data_entries[3]])
        tree.delete(*tree.get_children())
        data = sql_modul.sql_query_all("lab")
    ...
    tree = ttk.Treeview(frm, columns=("Laboratuvar", "Vergi Dairesi", "Şube", "IBAN"),
                        selectmode="browse")
    tree["show"] = "headings"
    tree.heading("Laboratuvar", text="Adı-Soyadı")
    tree.heading("Vergi Dairesi", text="Vergi Dairesi")
    tree.heading("Şube", text="Şube")
    tree.heading("IBAN", text="IBAN")

    for col in ["Laboratuvar", "Vergi Dairesi", "Şube", "IBAN"]:
    ...
    add_button = ttk.Button(frm2, text="Lab Ekle", command=add_member, padding=(15, 4))
